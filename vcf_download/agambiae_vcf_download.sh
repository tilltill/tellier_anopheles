...
for sample_set in "${sample_sets[@]}"; do
    ...
    if [[ -f "$csv_file" ]] && [[ $(wc -l <"$csv_file") -gt 1 ]]; then
        ...
        bcftools view -r 3R,3L $HOME/vcf_files/$sample_id.vcf.gz -o $HOME/vcf_files/combined_chr3R3L.vcf.gz || { echo "Failed to filter $sample_id.vcf.gz"; exit 1; }

        # Create index for the new file
        bcftools index $HOME/vcf_files/combined_chr3R3L.vcf.gz || { echo "Failed to index $sample_id.vcf.gz"; exit 1; }
        ...
        tail -n +3 temp_head.csv | while IFS=, read -r sample_id remainder; do
            ...
            bcftools merge $HOME/vcf_files/combined_chr3R3L.vcf.gz $HOME/vcf_files/$sample_id.vcf.gz -Oz -r 3R,3L -o $HOME/vcf_files/temp_combined_chr3R3L.vcf.gz || { echo "Failed to merge $sample_id.vcf.gz"; exit 1; }

            # Create index for the temp file
            bcftools index $HOME/vcf_files/temp_combined_chr3R3L.vcf.gz || { echo "Failed to index $sample_id.vcf.gz"; exit 1; }
            ...
            mv $HOME/vcf_files/temp_combined_chr3R3L.vcf.gz $HOME/vcf_files/combined_chr3R3L.vcf.gz
        done

        rm temp_head.csv
        rm $HOME/vcf_files/temp_combined_chr3R3L.vcf.gz
        rm $HOME/vcf_files/temp_combined_chr3R3L.vcf.gz.csi
    else
