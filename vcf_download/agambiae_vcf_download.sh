...
for sample_set in "${sample_sets[@]}"; do
    ...
    if [[ -f "$csv_file" ]] && [[ $(wc -l <"$csv_file") -gt 1 ]]; then
        ...
        bcftools view -r 3R,3L $HOME/vcf_files/test_download/$sample_id.vcf.gz -Oz -o $HOME/vcf_files/test_download/combined_chr3R3L.vcf.gz || { echo "Failed to filter $sample_id.vcf.gz"; exit 1; }

        # Create index for the new file
        bcftools index $HOME/vcf_files/test_download/combined_chr3R3L.vcf.gz || { echo "Failed to index $sample_id.vcf.gz"; exit 1; }
        ...
        tail -n +3 temp_head.csv | while IFS=, read -r sample_id remainder; do
            ...
            bcftools index -f $HOME/vcf_files/test_download/combined_chr3R3L.vcf.gz || { echo "Failed to index combined_chr3R3L.vcf.gz"; exit 1; }

            # Merge the new file with the combined file for chromosome 3R and 3L into a temporary file
            bcftools merge $HOME/vcf_files/test_download/combined_chr3R3L.vcf.gz $HOME/vcf_files/test_download/$sample_id.vcf.gz -r 3R,3L -Oz -o $HOME/vcf_files/test_download/temp_combined_chr3R3L.vcf.gz || { echo "Failed to merge combined_chr3R3L.vcf.gz"; exit 1; }
            ...
            mv $HOME/vcf_files/test_download/temp_combined_chr3R3L.vcf.gz $HOME/vcf_files/test_download/combined_chr3R3L.vcf.gz
        done

        rm $HOME/vcf_files/test_download/temp_combined_chr3R3L.vcf.gz
        rm $HOME/vcf_files/test_download/temp_combined_chr3R3L.vcf.gz.csi
    else
