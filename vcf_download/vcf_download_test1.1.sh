...
if [[ -f "$csv_file" ]] && [[ $(wc -l <"$csv_file") -gt 1 ]]; then
    # Get the first five lines of the metadata CSV file and save it temporarily
    ...
    tail -n +3 temp_head.csv | while IFS=, read -r sample_id remainder; do
        ...
        bcftools merge $HOME/vcf_files/test_download/combined_chr3R.vcf.gz $HOME/vcf_files/test_download/$sample_id.vcf.gz -Oz -r 3R -o $HOME/vcf_files/test_download/temp_combined_chr3R.vcf.gz || { echo "Failed to merge $sample_id.vcf.gz"; exit 1; }
