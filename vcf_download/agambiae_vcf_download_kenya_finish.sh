...
for sample_set in "${sample_sets[@]}"; do
    ...
    if [[ -f "$csv_file" ]] && [[ $(wc -l <"$csv_file") -gt 1 ]]; then
        ...
        # Read the temporary CSV file line by line starting from the thirteenth line (twelfth sample)
        tail -n +13 temp_head.csv | while IFS=, read -r sample_id remainder; do
